...
# Connection info
write_url = 'http://localhost:8086/write?db=printer&rp=autogen&precision=s'
print (write_url)
...
def main():
    ...
    for i in range(60*60*2+2):
        ...
        now += second
    print('Generated {} points to send into influx'.format(len(points)))
    data='\n'.join(points)
    print(data[0:740])
    #temperatures hotend=220.890908,bed=91.014744,air=77.614856 1585958400
    # Write data to Kapacitor
    r = requests.post(write_url, data)
    if r.status_code != 204:
        print(r.text, sys.stderr)
        return 1
    return 0
